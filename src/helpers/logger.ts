...
class LoggerVersionTwo {
    ...
    private log(level: LOGLEVEL, message: string, extra: string[]) {
        const entry = {
        ...
        if(extra === undefined /* || null */) {
            console.log(`${entry.ts} [${entry.level}] - ${entry.logger} - ${entry.message}`);
        } else {
...
export const getLogger = (loggerName: string) => {
    return new LoggerVersionTwo(loggerName);
}